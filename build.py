...
def afterCompile():
    ...
    execCommand("cd ../ && git log -1 | grep commit | cut -d ' ' -f 2 >  baiduNaviSDK/output/navi_ver", "generate version info")

    if navi_build_config["engine"]["source_build"]:
        execCommand("cd ../../../../../lib && git log -1 | grep commit | cut -d ' ' -f 2 >> ../app/naviapp/baidu-navi/ios/baiduNaviSDK/output/navi_ver && cd ../app/naviapp/baidu-navi/ios/baiduNaviSDK/ && date +%Y-%m-%d-%H:%M:%S >> output/navi_ver", "generate version info")
    else:
        execCommand("cat compile_output/navi_ver >> ../baiduNaviSDK/output/navi_ver && cd ../baiduNaviSDK/ && date +%Y-%m-%d-%H:%M:%S >> output/navi_ver", "generate version info")
    execCommand("cd ../baiduNaviSDK/output && cp navi_ver pub && zip -r pub.zip pub && rm -rf pub", "compress output", True)

    if navi_build_config["options"]["scan_code_risk"]["open"]:

	scanAllRisk()

	execCommand("mkdir ../baiduNaviSDK/output/AssistInfo && mv test_* ../baiduNaviSDK/output/AssistInfo && mv macro* ../baiduNaviSDK/output/AssistInfo", "scan code risk", True)
...
def setParamConfig():
    ...
    for option in navi_build_config["options"]:
        ...
        option1 = navi_build_config["options"][option]["short_option"]
        option2 = navi_build_config["options"][option]["long_option"]
        pAction = "store_true"
        pDest = "%s" % option
        pDefault = False
        pHelp = "open option %s" % option
        parser.add_option(option1, option2, action = pAction, dest = pDest, default = pDefault, help = pHelp)
